  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 sm:p-8">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  id="phoneNumber"
                  type="tel"
                  value={updateVendorInput.phone}
                  onChange={(e) => {
                    const value = e.target.value;
                    // Only allow digits and limit to 10
                    if (/^\d*$/.test(value) && value.length <= 10) {
                      updateInput('phone', value);
                    }
                  }}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                  maxLength={10}
              ...
              <div>
                ...
                <input
                  id="alternatePhone"
                  type="tel"
                  value={updateVendorInput.alternate_phone || ''}
                  onChange={(e) => {
                    const value = e.target.value;
                    // Only allow digits and limit to 10
                    if (/^\d*$/.test(value) && value.length <= 10) {
                      updateInput('alternate_phone', value || null);
                    }
                  }}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                  maxLength={10}
            ...
            <div>
              ...
              <textarea
                id="address"
                value={updateVendorInput.address}
              ...
              />
              <p className="text-gray-500 text-xs mt-1">{updateVendorInput.address.length}/200 characters</p>
            ...
            <div>
              ...
              <input
                id="gstNumber"
                type="text"
                value={updateVendorInput.gst_number || ''}
                onChange={(e) => updateInput('gst_number', e.target.value || null)}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
            ...
            <div>
              ...
              <textarea
                id="notes"
                value={updateVendorInput.notes || ''}
                onChange={(e) => updateInput('notes', e.target.value.slice(0, 100))}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                rows={2}
                maxLength={100}
                placeholder="Add any internal remarks"
              />
              <p className="text-gray-500 text-xs mt-1">{(updateVendorInput.notes || '').length}/100 characters</p>
            </div>
      ...
      {showFailurePopup && (
        <Popup
          title="Update Failed"
          message={errors.form || 'Failed to update vendor. Please try again.'}
